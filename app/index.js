...
const question_controller = require("./question/question_controller");
const user_uploader = require("./user/user_upload");

// Set up default mongoose connection
const mongo_user_string = config.get("database.user")
  ? `${config.get("database.user")}:${config.get("database.password")}@`
  : "";
const mongo_db = `mongodb://${mongo_user_string}${config.get(
  "database.host"
...
app.put(
  "/users/:id",
  accounts_controller.verifyAuthenticationMiddleWare,
  (req, res) => controller.mainControlller(user_controller.updateUser, req, res)
);

app.put(
  "/users/roles/:id/:role_name",
  accounts_controller.verifyAuthenticationMiddleWare,
  (req, res) =>
...
app.get("/partners/search", (req, res) =>
  controller.mainControlller(
    partner_controller.savePartnersForGeoQuery,
    req,
    res
  )
);
...
app.post(
  ...
  (req, res) =>
    controller.mainControlller(
      message_controller.receiveMessagesByBuddyForLoggedInUser,
      req,
      res
    )
);
...
app.post("/question", (req, res) =>
  controller.mainControlller(question_controller.saveQuestion, req, res)
);

app.delete("/question/:id", (req, res) =>
  controller.mainControlller(question_controller.deleteQuestion, req, res)
);

app.put("/question/:id", (req, res) =>
  controller.mainControlller(question_controller.saveReply, req, res)
);

app.put("/question/:id/:r_id", (req, res) =>
  controller.mainControlller(question_controller.deleteReply, req, res)
);

app.put("/question/:id/", (req, res) =>
  controller.mainControlller(question_controller.upvoteQuestion, req, res)
);

app.get("/question", (req, res) =>
  controller.mainControlller(question_controller.getAllQuestions, req, res)
);
...
app.post("/interactions/:confirmation_code", (req, res) =>
  controller.mainControlller(
    interactions_controller.confirmInteraction,
    req,
    res
  )
);
...
const server = app.listen(3001, () =>
  logger.info("Waitlist API listening on port 3001!")
);
...
io.of("/messaging").on("connection", socket =>
  co(message_controller.initSocketConnection(socket)).catch(err =>
    logger.error(err)
  )
);
