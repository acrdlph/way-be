...
passport.use(
  new LocalStrategy(
    ...
    (loginname, password, done) => {
      co(passportLoginCallBack(loginname, password, done)).catch(err => {
        done(err);
      });
    }
  )
...
exports.signUp = function*(req, res) {
  ...
  if (!username || !validator.isEmail(email) || !password) {
    throw error_util.createError(
      400,
      "Username, Email(check format) and Password are required"
    );
  }
  const existing_username = yield user_repository.getUserForUsername(username);
  const existing_id_username = yield user_repository.getUserForUsername(
    user_id
  );
  ...
  const password_hash = yield auth_util.getPasswordHash(
    password,
  ...
  if (user) {
    ...
    user = yield user_repository.createNewRegisteredUser(
      username,
      email,
      password_hash
    );
...
exports.addLoggedInUsers = function*(req, res) {
  if (!loggedInUsers.includes(req.params.id)) {
    loggedInUsers.push(req.params.id);
  }
  console.log(loggedInUsers);
};

exports.logout = function*(req, res) {
  console.log(req.params.id, "edo trexi");
  loggedInUsers = loggedInUsers.filter(u => u !== req.params.id);
  console.log(loggedInUsers);
  res.json({
    auth: false,
    token: null
...
exports.verifyAuthenticationMiddleWare = (req, res, next) =>
  co(verifyAuthentication(req, res, next)).catch(err =>
    error_util.handleError(req, res, err, { auth: false })
  );
...
function* verifyAuthentication(req, res, next) {
  ...
  try {
    const decoded_user = yield auth_util.verifyJwt(
      token,
      config.get("server.private_key")
    );
